...
识别物品少：选“更多物品”
服务器很弱：选“弱 CPU 快速”
想看每个检测框：输出方式选“检查模式：显示每个实例”
想自己调参数：选“自定义”，再打开高级参数
```
...
注意：强力去重会压掉重叠很高的不同标签，结果更干净，但可能误删一些真实物品。
...
2. **同类物品默认合并**：图里有 5 本书，学习模式下只显示一行 `book`，但 `Count` 会显示数量。
